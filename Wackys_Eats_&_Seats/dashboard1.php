...
$query = "SELECT * FROM events_reservation WHERE id LIKE '%$search%' ORDER BY id LIMIT $start, $records_per_page";
$query_run = mysqli_query($con, $query);

// Count total records for pagination
$total_records = mysqli_fetch_assoc(mysqli_query($con, "SELECT COUNT(*) AS total FROM events_reservation WHERE id LIKE '%$search%'"))['total'];
...
<body>
    ...
    <h3 class="sub-heading"> Wacky's Receptions Reservations </h3> 
    <div class="reception-container">
        <header>
            <div class="reception-filterEntries">
                <div class="reception-filter">
                    <label for="search"> Search: </label>
                    <input type="search" id="search" placeholder="Enter Reservation ID">
                </div>
            </div>
        </header>
                ...
                if(mysqli_num_rows($query_run) > 0)
                {
                    foreach($query_run as $customer)
                    {
                        ...
                        <tr>
                            ...
                            <td><?=  $customer['theme_event']; ?></td>
                            <td><?=  $customer['theme_cake']; ?></td>
                            <td><?=  $customer['dedication_message']; ?></td>
                            <td><?=  $customer['requests']; ?></td>

                           
                            <td>
                                <div class="crudbutton">
                                    <a href="customer-view1.php?id=<?= $customer['id']; ?>"><button><i class="fa-regular fa-eye"></i></button></a>
